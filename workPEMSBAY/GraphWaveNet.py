...
class gwnet(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.blocks * self.layers):
            ...
            # residual_reshaped = residual.view(residual.size(0), residual.size(1), -1)  # Flatten nodes and sequence length
            # gate = self.gate_convs[i](residual_reshaped)
            # gate = gate.view(residual.size(0), -1, residual.size(2), residual.size(3))  # Reshape back if needed
            # print("Shape before conv1d:", residual.shape)
            # print("Shape after reshape:", residual_reshaped.shape)
            ...
            s = x
            # s = s.view(s.size(0), s.size(1), -1)  # Reshape to 3D: [batch_size, channels, height*width]
